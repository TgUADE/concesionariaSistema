...
public class FormaDePagoFactoryImpl implements InterfazFormaDePagoFactory {
    @Override
    public IFormaDePago crearFormaDePago(TipoFormaPago tipo) { // Removed throws
        if (tipo == null) {
            throw new RuntimeException("El tipo de forma de pago no puede ser nulo.");
        }
        switch (tipo) {
            case CONTADO:
            ...
            default:
                throw new RuntimeException("Tipo de forma de pago no soportado: " + tipo);
        }
    }
}
