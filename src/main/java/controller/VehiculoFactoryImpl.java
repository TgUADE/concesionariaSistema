...
public class VehiculoFactoryImpl implements InterfazVehiculoFactory {
    ...
    public Vehiculo crearVehiculo(
                        ...
                        String chasis,
                        String motor,
                        boolean disponibleVenta) { // Removed throws

        // Basic validation (can also be in Vehiculo constructor or a dedicated validation service)
        if (marca == null || marca.trim().isEmpty()) {
            throw new RuntimeException("Marca es obligatoria para crear un vehículo.");
        }
        if (modelo == null || modelo.trim().isEmpty()) {
            throw new RuntimeException("Modelo es obligatorio para crear un vehículo.");
        }
        if (tipo == null) {
            throw new RuntimeException("Tipo es obligatorio para crear un vehículo.");
        }
        if (precioBase < 0) {
            throw new RuntimeException("Precio base no puede ser negativo.");
        }
