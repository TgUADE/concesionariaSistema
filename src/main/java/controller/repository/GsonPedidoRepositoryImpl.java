...
public class GsonPedidoRepositoryImpl implements InterfazPedidoRepository {
    ...
    @Override
    public List<Pedido> findPedidosByFechaAndEstadoAndCliente(Date fechaDesde, Date fechaHasta, String estado, int clienteId) {
        return pedidos.stream()
                .filter(pedido -> {
                    // Check date range
                    if (fechaDesde != null && pedido.getFechaCreacion() != null && pedido.getFechaCreacion().before(fechaDesde)) {
                        return false;
                    }
                    if (fechaHasta != null && pedido.getFechaCreacion() != null && pedido.getFechaCreacion().after(fechaHasta)) {
                        return false;
                    }
                    // Check estado
                    if (estado != null && pedido.getEstadoActual() != null && 
                        !pedido.getEstadoActual().getClass().getSimpleName().toLowerCase().contains(estado.toLowerCase())) {
                        return false;
                    }
                    // Check cliente ID
                    if (clienteId > 0 && pedido.getCliente() != null && pedido.getCliente().getId() != clienteId) {
                        return false;
                    }
                    return true;
                })
    ...
    @Override
    public List<Pedido> findPedidosByFechaAndEstado(Date fechaDesde, Date fechaHasta, String estado) {
        return pedidos.stream()
                .filter(pedido -> {
                    // Check date range
                    if (fechaDesde != null && pedido.getFechaCreacion() != null && pedido.getFechaCreacion().before(fechaDesde)) {
